...
FEED_URL = f"https://calendar.google.com/calendar/ical/{urllib.parse.quote(CALENDAR_ID, safe='')}/public/basic.ics"
...
def main():
    ...
    for line in lines:
        ...
        if line == "END:VEVENT" and current is not None:
            if (
                current.get("start")
                and is_future(current["start"])
                and "waiata" in current.get("summary", "").lower()
            ):
